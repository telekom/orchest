...
/**
 * Executor for Manual Tasks. Passes through immediately with no automated logic, applying data
 * mappings.
 */
public class ManualTaskExecutor implements NodeExecutor {
  private static final Logger log = LoggerFactory.getLogger(ManualTaskExecutor.class);

  /**
   * Executes the manual task by applying data mappings and proceeding to outgoing nodes.
   *
   * @param instance the active process instance
   * @param node the manual task node to execute
   * @param context the execution context
   */
  @Override
  public void execute(ProcessInstance instance, BaseNode node, ExecutionContext context) {
    log.info("Executing Manual Task: {}. (No automated logic)", node.getName());
